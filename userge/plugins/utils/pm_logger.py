...
async def pm_logger_(message: Message):
    ...
    await SAVED_SETTINGS.update_one(
        {'_id': 'PM_LOGGING'},
        {"$set": {'is_active': Config.PM_LOGGING}},
        upsert=True
    )


@userge.on_message(
    filters.private & filters.incoming & ~filters.me & ~filters.service &
    ~filters.bot & ~filters.edited & ~allowAllFilter & allowPmLoggingFilter
)
async def pm_logger(_, message: Message):
    ...
    if len(PM_LOGGER_CACHE) == 0:  # First Pm
        logger_msg_count = await userge.send_message(
            ...
            new_pm_logger.format(
                u_id,
                mention_html(u_id, u_name)
            ),
    ...
    # either it can be the same user or a new user
        ...
        if u_id in PM_LOGGER_CACHE:  # if same person just updating the count
            ...
            u_mention = mention_html(
                u_info_id,
                PM_LOGGER_CACHE[u_info_id]['name']
            )
            ...
            try:
                await userge.edit_message_text(
                    ...
                    edit_pm_logger.format(
                        u_info_id,
                        u_mention,
                        PM_LOGGER_CACHE[u_info_id]['msg_count']
                    )
                )
