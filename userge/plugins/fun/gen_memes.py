...
@userge.on_cmd("gm", about={
    ...
    'examples': [
        "{tr}gm Hi ; Hello",
        "{tr}gm -m32 Hi ; Hello"],
    'Memes': "<a href='https://telegra.ph/Meme-Choices-10-01'>"
             "<b>See MEME List</b></a>"})
async def gen_meme(message: Message):
    ...
    if not (Config.IMGFLIP_ID or Config.IMGFLIP_PASS):
        return await message.edit(
                   ...
                   "`IMGFLIP_PASS` = password via **https://imgflip.com/**"
               )
    ...
    if ";" in text:
        ...
        return await message.err(
                   "Invalid Input! Check help for more info!",
                   del_in=5
               )
    ...
    if '-m' in message.flags:
        ...
        if numb in range(93):
            ...
            return await message.err(
                       "Choose a number between (0 - 93) only !",
                       del_in=5
                   )
    else:
    ...
    await message.edit(
        f"<code>Generating a meme for ...</code>\n{meme_choice['name']}"
    )
    ...
    if not response['success']:
        return await message.err(
                   f"<code>{response['error_message']}</code>", del_in=5
               )
