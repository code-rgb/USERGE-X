...
no_input_reply = "I don't know who you're talking about, you're going to need to specify a user...!"
...
async def warn_func(message: Message):
    ...
    wcount = await WARNS_DB.count_documents({'chat_id': message.chat.id, 'user_id': warned_user.id})
...
"""
    warn_id = str((await WARNS_DB.insert_one({
    ...
    if userge.has_bot and messaage.client.is_bot:
        btn_row = [InlineKeyboardButton("⚠️  Remove Warn", callback_data=f"remove_warn_{warn_id}")]
        if rules:
...
async def warn_mode(message: Message):
    ...
    result = await WARN_DATA.update_one(
        {"chat_id": chat_id}, {"$set": {"warn_mode": warn_mode}}, upsert=True
    )
    out = "{} <b>{}</b> for Chat: {}"
...
async def maxwarns(message: Message):
    ...
    result = await WARN_DATA.update_one(
        {"chat_id": chat_id}, {"$set": {"rules": int(maxwarns)}}, upsert=True
    )
    out = "{} <b>{}</b> for Chat: {}"
    if result.upserted_id:
...
async def chat_rules(message: Message):
    ...
    result = await WARN_DATA.update_one(
        {"chat_id": chat_id}, {"$set": {"rules": rules}}, upsert=True
    )
    out = "{} <b>{}</b> for Chat: {}"
    if result.upserted_id:
