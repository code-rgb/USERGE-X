...
import asyncio
from userge import get_collection, Message, userge, Config
...
async def delfed_(message: Message):
    ...
    if '-all' in message.flags:
        ...
        if found:
            ...
            return await message.err(out + "**Does't exist in your Fed List !**", del_in=5)
    await CHANNEL.log(msg_)
...
async def fban_(message: Message):
    ...
    if user in Config.SUDO_USERS or user in Config.OWNER_ID or user == (await message.client.get_me()).id:
        return await message.err("Can't F-Ban users that exists in Sudo or Owners", del_in=5)
    failed = []
    total = 0
    ...
    async for data in FED_LIST.find():
        ...
        try:
            async with userge.conversation(chat_id, timeout=15) as conv:
                ...
                response = await conv.get_response(mark_read=True, filters=(filters.user([609517172]) & ~filters.service))
                resp = response.text
                if (
...
async def fban_lst_(message: Message):
    ...
    await message.edit_or_send_as_file("**Connected federations:**\n\n" + out if out else "**You haven't connected to any federations yet!**", caption="Connected Fed List")
