...
async def dset_(message: Message) -> None:
    ...
    try:
        ...
        await message.edit(
            f"set `{path}` as **working directory** successfully!", del_in=5
        )
...
@userge.on_cmd(
    "dreset",
    about={"header": "reset to default working directory", "usage": "{tr}dreset"},
    allow_channels=False,
)
async def dreset_(message: Message) -> None:
    ...
    await message.edit(
        f"reset **working directory** to `{path}` successfully!", del_in=5
    )


@userge.on_cmd(
    "dclear",
    about={"header": "Clear the current working directory"},
    allow_channels=False,
)
...
async def drename_(message: Message) -> None:
    ...
    await message.edit(
        f"path : `{path}` **renamed** to `{new_path}` successfully!", del_in=5
    )
